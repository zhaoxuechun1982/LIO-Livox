 ...
 * @param[out] frames Sliding window buffer of lidar frames with IMU pre-integration
...
private:
   ...
   * @param [in] frames Sliding lidar frame window
   * @param [in] q_wg Coarse gravity rotation quaternion
   * @param [in] prior_r Prior rotation manifold value
   * @param [in] prior_v_list Prior velocity list of all frames
   * @param[in] ex_r_lb Extrinsic rotation matrix, transform point from lidar frame to IMU body frame
   * @param[in] ex_t_bl Extrinsic translation, transform point from IMU body frame to lidar frame
   ...
   */
  bool write_optimization(std::deque<LidarFrame>& frames, Eigen::Vector3d& g_b);
