...
namespace lio
{
  PoseEstimator::PoseEstimator(const float& filter_corner, const float& filter_surface)
  {
    ...
    laserCloudCornerLast.resize(silde_windows_size);
    for(auto& p:laserCloudCornerLast)
      p.reset(new pcl::PointCloud<PointType>);
    laserCloudSurfLast.resize(silde_windows_size);
    for(auto& p:laserCloudSurfLast)
      p.reset(new pcl::PointCloud<PointType>);
    laserCloudNonFeatureLast.resize(silde_windows_size);
    for(auto& p:laserCloudNonFeatureLast)
      p.reset(new pcl::PointCloud<PointType>);
    laserCloudCornerStack.resize(silde_windows_size);
    for(auto& p:laserCloudCornerStack)
      p.reset(new pcl::PointCloud<PointType>);
    laserCloudSurfStack.resize(silde_windows_size);
    for(auto& p:laserCloudSurfStack)
      p.reset(new pcl::PointCloud<PointType>);
    laserCloudNonFeatureStack.resize(silde_windows_size);
    ...
    map_manager = new MAP_MANAGER(filter_corner, filter_surface);
    threadMap = std::thread(&PoseEstimator::threadMapIncrement, this);
...
[[noreturn]] void PoseEstimator::threadMapIncrement()
{
...
void PoseEstimator::vector2double(const std::list<LidarFrame>& lidarFrameList){
  int i = 0;
...
void PoseEstimator::double2vector(std::list<LidarFrame>& lidarFrameList){
  int i = 0;
...
void PoseEstimator::EstimateLidarPose(std::list<LidarFrame>& lidarFrameList,
                           const Eigen::Matrix4d& exTlb,
...
void PoseEstimator::Estimate(std::list<LidarFrame>& lidarFrameList,
  ...
  for(int i = 0; i < 4851; i++){
    ...
    NonFeatureKdMap[i] = mapPoseEstimator::PoseEstimator(const float& filter_corner, const float& filter_surface)
{
  laserCloudCornerFromLocal.reset(new pcl::PointCloud<PointType>);
  laserCloudSurfFromLocal.reset(new pcl::PointCloud<PointType>);
  laserCloudNonFeatureFromLocal.reset(new pcl::PointCloud<PointType>);
...
void PoseEstimator::Estimate(std::list<LidarFrame>& lidarFrameList,
  ...
  if(windowSize == silde_windows_size) {
...
_manager->getNonFeatureKdMap(i);
  ...
  for(int iterOpt=0; iterOpt<max_iters; ++iterOpt){
    ...
    if (deltaR < 0.05 && deltaT < 0.05 || (iterOpt+1) == max_iters){
      ROS_INFO("Frame: %d\n",frame_count++);
      if(windowSize != silde_windows_size) break;
      ...
      for (int i = 1; i < silde_windows_size; i++)
      {
...
void PoseEstimator::MapIncrementLocal(const pcl::PointCloud<PointType>::Ptr& laserCloudCornerStack,
...
bool PoseEstimator::run_imu_align(std::deque<LidarFrame>& frames,
                              Eigen::Vector3d& g_b,
                              const Eigen::Matrix3d& ex_r_lb, 
                              const Eigen::Vector3d& ex_t_bl)
{
  return imu_aligner_ptr_->instance().run_align(frames,g_b, ex_r_lb, ex_t_bl, silde_windows_size);
}
