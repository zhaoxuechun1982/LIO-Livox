                           ...
                           const int& win_size)
{
  ...
  if (!solve_ceres_problem(frames, q_wg, prior_r, prior_v_list, ex_r_lb, ex_t_bl)) {
    return false;
  }
...
{  
  Eigen::Vector3d avg_acc = -frames.begin()->imu_integrator.GetAverageAcc();
                                     ...
                                     const Eigen::Vector3d& ex_t_bl)
{
  ...
  for(size_t i = 1; i < v_list_size; i++) {
    ...
    problem.AddResidualBlock(Cost_Initialization_IMU::Create(frames[i].imu_integrator,
                             ri,
                             rj,
                             pj-pi,
                             Eigen::LLT<Eigen::Matrix<double, 9, 9>>
                             (frames[i].imu_integrator.GetCovariance().block<9,9>(0,0).inverse())
                             .matrixL().transpose()),
                             nullptr,
                             para_r,
                             para_v_list[i-1],
                                        ...
                                        const int& win_size) const
{
  ...
  for(size_t i = 0; i < frames.size() - 1; i++) {
    frames[i+1].imu_integrator.PreIntegration(frames[i].timestamp, frames[i].bg, frames[i].ba);
  }
